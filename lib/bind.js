...
(function(exports) {
    ...
    function bind(elem, model, mapper) {
        if (!elem) console.error("bind.js: No element was passed!");
    ...
    function clone(elem, model, mapper) {
        ...
        for (var i = 0; i < CACHE.length; i++) {
            if (CACHE[i].elem == elem && CACHE[i].model == model) pos = i;
        }
        if (pos == -1) {
    ...
    function setContent(elem, value) {
        ...
        if (typeof(value) == "undefined") elem.parentNode.removeChild(elem);
        else if (value == bind.YES) return elem;
        else if (value === bind.NO) elem.parentNode.removeChild(elem);
    ...
    function digest() {
        ...
        for (var i = 0; i < CACHE.length; i++) {
            ...
            if (cache.modelRep != JSON.stringify(cache.model)) {
                console.log("ABOUT TO RE-BIND, MODEL: "+JSON.stringify(cache.model));
                //console.log("ELEM: "+cache.elem);
                //console.log("MAPPER: "+cache.mapper);
    ...
    bind.digest = digest;
    if (typeof(window) != "undefined") window.setInterval(digest, 1000);
