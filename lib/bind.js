...
(function(exports) {
    ...
    function map(elem, mapping) {
        ...
        for (var key in mapping) {
            ...
            if (ak) for (var i = 0; i < len; i++) set(nodes[i], ak, value);
            ...
            else for (var i = 0; i < len; i++) set(nodes[i], null, value);
        }
    ...
    function set(elem, attr, value) {
        ...
        if (attr) {
            ...
            */
            if (isBoolean(value)) { if (!value) elem.parentNode.removeChild(elem); }
            else if (isObject(value)) elem.textContent = value.toString();
            else if (Array.isArray(value)) elem.textContent = value.toString();
